...
# -----------------------------------------------------------------------------
# Training Mean, Std, First Peak Magnitude
# -----------------------------------------------------------------------------
def extract_summary_stats(df):
    q = df.values.flatten()
    mean_q = np.mean(q)
    std_q = np.std(q)
    first_peak_region = q[15:21] 
    first_peak_magnitude = np.max(first_peak_region)
    return torch.tensor([mean_q, std_q, first_peak_magnitude], dtype=torch.float)
...
# -----------------------------------------------------------------------------
# Generate Summary Vectors for Simulated and Observed Streamflow
# -----------------------------------------------------------------------------
gage_id = "01608500"
sim_data = []
...
for i in range(num_sims):
    ...
    if i == 0:
        ...
        summary_obs = extract_summary_stats(obsv_df[[gage_id]])
    ...
    features_sim = extract_summary_stats(q_sim_df)
