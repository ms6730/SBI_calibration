...
# -----------------------------------------------------------------------------------
# Training includes: Summary Statistics (mean, std, magnitude) + Full Streamflow Data
# -----------------------------------------------------------------------------------
def extract_summary_stats_and_series(df):
    ...
    mean_q = np.mean(q)
    std_q = np.std(q)
    first_peak_region = q[15:21]  # where the first peak happens
    first_peak_magnitude = np.max(first_peak_region)
    feature_vector = np.concatenate(([mean_q, std_q, first_peak_magnitude], q))
...
# -----------------------------------------------------------------------------
# Generate Summary + Streamflow Features for Simulated and Observed Streamflow
# -----------------------------------------------------------------------------
gage_id = "01608500"
sim_data = []
...
for i in range(num_sims):
    ...
    # Add noise to simulated streamflow
    ...
    # Extract summary stats + full streamflow
